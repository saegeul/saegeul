...
class Member extends MX_Controller
{
	...
	function send_again()
	{
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			$this->load->view('member/send_again_form', $data);
		}
	}
	...
	function activate_invited()
	{
		...
		if ($this->tank_auth->activate_user($user_id, $new_email_key)) {		// success
			...
			$this->users->req_change_password($user_id);
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->load->view('member/forgot_password_form', $data);
		}
	}
	...
	function reset_password()
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$this->_show_message($this->lang->line('auth_message_new_password_activated').' '.anchor('/member/login/', 'Login'));
	...
	*/
	function change_password()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			redirect('/member/admin/member/do_login/');
	...
	function change_email()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->load->view('member/change_email_form', $data);
		}
	}

	/**
	 * Replace user email with a new one.
	 * User is verified by user_id and aumembertication code in the URL.
	 * Can be called by clicking on link in mail.
	 *
	 * @return void
	 */
	function reset_email()
	{
		...
		if ($this->tank_auth->activate_new_email($user_id, $new_email_key)) {	// success
			$this->tank_auth->logout();
			$this->_show_message($this->lang->line('auth_message_new_email_activated').' '.anchor('/member/login/', 'Login'));
	...
	function unregister()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->load->view('member/unregister_form', $data);
		}
	}
	...
	function do_invite(){ 
		...
		if ($this->form_validation->run()) {										if (!is_null($data = $this->tank_auth->create_user(
				...
				if ($email_activation) {									// send "invite" email
					$data['activation_period'] = $this->config->item('email_activation_expire', 'tank_auth') / 3600;


						
						
					$this->_send_email('invite', $data['email'], $data);
